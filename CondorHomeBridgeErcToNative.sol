...
contract CondorHomeBridgeErcToNative is Context, AccessControl, ReentrancyGuard, Pausable {
    
    using SafeMath for uint256;
    
    address payable public condor;
    ...
    constructor(uint256 _devFee, uint256 _burnFee, uint256 _bankFee, address _devWallet, address _burnWallet, address _bankWallet, address _auditorWallet, address _gatekeeperWallet) {
        condor = payable(address(this));
        ...
        devWallet = payable(address(_devWallet));
        burnWallet = payable(address(_burnWallet));
        bankWallet = payable(address(_bankWallet));
    ...
    function rescueFunds() external payable nonReentrant {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "must have admin role");
        require(bankWallet.send(address(this).balance), 'cannot transfer tokens to bridge contract');
    }
    
    function deposit(address _to, uint256[] memory _amounts, bytes[] memory _signatures) external payable nonReentrant whenNotPaused {
       ...
       TransactionInfo memory transaction = TransactionInfo({
        ...
        require(msg.value == transaction.amount, 'transaction amount doesnt match');
        require(transaction.amount <= address(_msgSender()).balance, 'dont have enough balance');
        ...
        require(devWallet.send(transaction.devFeeAmount), 'cannot transfer dev fee');
        require(burnWallet.send(transaction.burnFeeAmount), 'cannot transfer burn fee');
        require(bankWallet.send(transaction.bankFeeAmount), 'cannot transfer bank fee');
        require(condor.send(transaction.amount.sub(transaction.totalFeeAmount)), 'cannot transfer tokens to bridge contract');
    ...
    function withdraw(address _to, uint256[] memory _amounts, bytes[] memory _signatures) external payable nonReentrant whenNotPaused {
       ...
       TransactionInfo memory transaction = TransactionInfo({
        ...
        require(payable(transaction.to).send(transaction.amount.sub(transaction.totalFeeAmount)), 'cannot transfer tokens to user');
